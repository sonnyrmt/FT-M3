...
server.get('/posts/:author/:title', (req,res) => {
  ...
  const filtered = posts.filter( p => p.author === author && p.title === title);
...
server.put('/posts', (req,res) => {
  ...
  if(id && title && contents) {
    ...
    if(matchId) {
      ...
      res.status(STATUS_USER_ERROR).json({error: "No se recibio el id necesario para modificar el Post"})
    }
  } else {
